function [v, crmax, selxtra, col] = ...
    collider(v, crmax, tau, selxtra, coeff, sD)

% collider - function to process collisions in cells
% [v, crmax, selxtra, col] = collider(v, crmax, tau, selxtra, coeff, sD)
% Inputs
%       v           Velocities of the particles
%       crmax       Estimated maximum relative speed in a cell (used excited value)
%       tau         Time step
%       selxtra     Extra selections carried over from last time step
...
for jcell = 1:ncell
    ...
    if (number > 1)
        ...
        % Determine number of candidate collision pairs to be selected in
        % this cell
        ...
        for isel = 1:nsel
           ...
           if(cr/crmax(jcell) > rand(1) )
               ...
               vcm = 0.5*(v(ip1, :) + v(ip2,:));    % Centre of mass vel.
