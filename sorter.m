function sD = sorter(x, y, L, dl, sD)

% sorter - Function to sort particles into cells
% sD = sorter(x, y, L, dl, sD)
%
% Inputs
...
% Find the cell address for each particles
npart = sD.npart;
ncell = sD.ncell;

jx = 1 + floor(x/dl) + round(L/dl) * floor(y/dl);
...
% Build index list as cumulative sum of the number of particles in each
% cell.
m = 1;
for jcell = 1:ncell
    sD.index(jcell) = m;
